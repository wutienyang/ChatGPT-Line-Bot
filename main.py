...
def run(event):
  ...
  if text.startswith("/"):
    ...
    line_bot_api.push_message(line_user_id,
                              TextSendMessage(text=f"prompt : {prompt}"))
...
def handle_audio_message(event):
  audio_name = f"{os.path.abspath(os.getcwd())}/whisper_audio.m4a"
  message_id = event.message.id
  message_content = line_bot_api.get_message_content(message_id)
  with open(audio_name, "wb") as f:
    f.write(message_content.content)
  text = whisper.generate(audio_name)
  response = chatgpt.get_response(event.source.user_id,
                                  f"{BASE_PROMPT} the sentences are {text}")
  line_bot_api.push_message(line_user_id,
                            TextSendMessage(text=f"Audio : {text}"))
